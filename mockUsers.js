...
const generateUsers = async (count = 100) => {
  ...
  const token = 'BQBTnxY0XxK5jQbg_Jq4rnR6_q6Nnk8ce73AABam1Rj3DJSvPr8ngU52f1f42YuFcvdLtETGJt-EMlZOJ1ILFd32GzzlgrFDWxtQO_ajryNMx6pvTjIU-itMwte_qATBr78XCmEOip_zizUO9irbFrPhwFYU6gPkHcQIkHhpw71GkqEhG_TSFzZK8PyjUK2XMl5ntZ498SvLszdwfc7NL2QhmDue1d4LgAR-zEqxOMyYAwxvkB8XMymY-zJrS1D8wLzvL4c';

  for (const user of users) {
    const genres = new Set();
    for (const song of user.profileSongs) {
      
      const res = await fetch(
        `https://api.spotify.com/v1/artists/${song.artistId}`,
        {
    ...
    user.genres = Array.from(genres);
  }
  return users;
};

(async () => {
  const Users = JSON.stringify(await generateUsers());
  
  fs.writeFile('users.txt', Users, 'utf8', (err) => {
    if (err) {
